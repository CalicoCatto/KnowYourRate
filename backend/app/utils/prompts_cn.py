...
1.【互动价值】如果互动率高于垂类平均，量化额外互动数 = (互动率差值/100) × 平均播放量。
...
4.【内容稳定性】如果中位数播放量接近平均播放量，说明内容表现稳定，品牌投放风险低。

5.【增长溢价】按当前增长率，3个月后同一个达人的报价将上涨约X%，现在合作性价比最高。

6.【使用权价值】如果品牌要求信息流投放权，计算替代成本（品牌自行拍摄广告素材约¥5,000-¥50,000）。

7.【内容长尾】（仅B站）B站内容的长尾效应意味着品牌曝光将持续6-12个月，等效CPM远低于账面。

8.【季节性时机】如果当前或即将进入旺季，达人档期紧张，理应溢价。
...
6.【直播带货实际ROI】如涉及直播带货，GMV ≠ 利润。\
需计算退货、佣金、坑位费后的实际ROI。

7.【隐性成本】品牌方的时间投入（brief制作、内容审核、沟通成本）也应计入总成本。

8.【受众重合度风险】如果品牌同时投放多个同垂类达人，受众重合度可能导致CPM虚高。
...
4. 走平台/私下双报价（如适用）
5. 置信度评分（0-1）
6. 如果Bull和Bear差距>80%，标记高不确定性
7. 价格锚定策略建议：
   - "高开策略"：当数据强势且有历史成交支撑时，锚定价上浮30%
   - "市场价策略"：当数据一般或首次合作时，直接报fair_market
   - "竞争策略"：当品牌有多个备选达人时，报fair_market下浮10%
...
{{
  ...
  "pricing_strategy": "高开策略" | "市场价策略" | "竞争策略",
  "pricing_strategy_reason": "选择该策略的原因",
...
7.【谈判话术】提供4个场景的模板回复：
   ...
   - 品牌说报价太高
   - 品牌要求走私下不走平台
8.【税务提醒】简述不同收款方式的税务影响，展示三种场景的到手金额对比
9.【时机建议】季节性因素和最佳合作时机
10.【数据质量声明】透明披露数据来源和可信度，让创作者了解建议的可靠程度
